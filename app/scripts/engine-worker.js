    ...
    function () {
      ...
      onmessage = function (param) {
        ...
        if (data.action === "draw") {
          ...
          if (data.quality === 200) {
            engine.drawTileOnBuffer(data.tile, data.model);
          } else if (data.quality === 201) {
            engine.drawTileOnBufferSetOnly(data.tile, data.model);
          } else if (data.quality === 100) {
            engine.drawSubTileOnBuffer(data.tile, data.model);
          } else if (data.quality === 300) {
            engine.drawSuperTileOnBuffer(data.tile, data.model);
          } else {
            throw "invalid drawing quality";
          }
          //setTimeout(function() {
          postMessage({
            action: "endTile",

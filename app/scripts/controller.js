...
FractalJS.Controller = function (fractal, model) {
  ...
  var pan = function (ratiox, ratioy) {
    var vsx = ratiox * camera.width,
      vsy = -ratioy * camera.height;
    var pc1 = camera.S2C(0, 0),
      pc2 = camera.S2C(vsx, vsy); // [pc1,pc2] is the movement vector on C
  ...
  var zoom = function (psx, psy, delta) {
    ...
    var vc = {
      x: camera.x - pc.x,
      y: camera.y - pc.y
    }; // vector to complex point at center
    ...
    util.loadJs("libs/hammerjs/hammer.min.js", function () {
      ...
      hammertime.on('panmove', function (ev) {
        ...
        if (isDragging) {
          ...
          var pc1 = camStart.S2C(0, 0),
            pc2 = camStart.S2C(vsx, vsy); // [pc1,pc2] is the movement vector on C
  ...
  if (params.keyboardControl) {
    ...
    document.onkeydown = function (e) {
      ...
      switch (keyCode) {
      case 107:
        zoom(canvas.width / 2, canvas.height / 2, 1 / (1 + ZOOM * modifier));
        break; // key +, zoom in
      case 109:
        zoom(canvas.width / 2, canvas.height / 2, 1 + ZOOM * modifier);
        break; // key -, zoom out
  ...
  function sendMouseMoveEvent() {
    var psx = lastMouseOnCanvasX,
      psy = lastMouseOnCanvasY;
    ...
    events.send("mouse.move", {
      ...
      cx: pc.x,
      cy: pc.y,
      iter: fractal.getIterAt(psx, psy)
    });
  }
